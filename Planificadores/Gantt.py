...
def calcular_metricas(df, RoundRobin=False):
    ...
    if RoundRobin:
        ...
        df_first_execution = df_sorted.drop_duplicates(
            subset='ID Proceso', keep='first').copy()
        # Calcular el tiempo de respuesta como la diferencia entre 'Inicio de Ejecución' y 'Tiempo de Llegada'
        df_first_execution['Tiempo de Respuesta'] = df_first_execution['Inicio de Ejecución'] - \
    ...
    tiempo_total_observado = df['Fin de Ejecución'].max(
    ) - df['Tiempo de Llegada'].min()
    ...
    if RoundRobin:
        ...
        tiempo_total_actividad_cpu = df['Fin de Ejecución'].max(
        ) - df['Inicio de Ejecución'].min()
    else:
...
def imprimir_metricas(metrics):
    print(
        f"Tiempo de Espera Promedio: {metrics['tiempo_espera_promedio']:.2f} unidades de tiempo")
    print(
        f"Tiempo de Retorno Promedio: {metrics['tiempo_retorno_promedio']:.2f} unidades de tiempo")
    print(
        f"Tiempo de Respuesta Promedio: {metrics['tiempo_respuesta_promedio']:.2f} unidades de tiempo")
    print(
        f"Variabilidad del Tiempo de Espera: {metrics['var_tiempo_espera']:.2f}")
    print(
        f"Máximo Tiempo de Espera: {metrics['max_tiempo_espera']} unidades de tiempo")
    print(f"Utilización de la CPU: {metrics['utilizacion_cpu']:.2f}%")
    print(
        f"Rendimiento: {metrics['rendimiento']:.2f} procesos/unidad de tiempo")
...
parser = argparse.ArgumentParser(
    description='Genera un diagrama de Gantt a partir de un archivo CSV de procesos.')
parser.add_argument(
    'file', type=str, help='Ruta al archivo CSV que contiene los datos de los procesos.')
...
if (file == 'data/rr_procesos.csv'):
    generar_diagrama_gantt(
        df, 'Diagrama de Gantt para Planificador Round Robin')
else:
    if (file == 'data/fcfs_procesos.csv'):
        ...
        generar_diagrama_gantt(
            df, 'Diagrama de Gantt para Planificador por Prioridad')
